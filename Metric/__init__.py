...
class MetricCenter():
    def __init__(self, opt, dev):
        self.metric_list = opt['metric'].split('-')
    ...
    def get_result(self):
        ...
        for metric_name, metric in zip(metric_list, self.metric):
            metric_results[metric_name] = metric.get_result()
...
class PSNR():
    ...
    def get_result(self):
        result = np.array(self.values_bucket).mean()
        self.values_bucket.clear()
        return result
...
class FID():
    ...
    def get_result(self):
        ...
        result = self.calculate_frechet_distance(mu_target, std_target, mu_output, std_output)
